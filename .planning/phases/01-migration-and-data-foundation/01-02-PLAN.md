...
must_haves:
  truths:
    ...
    - "No file in electron-app/src/ imports directly from the 'zustand' package"
...
<objective>
Remove all Zustand store shim files and update every import across the codebase to point directly at the React Context source files. Delete the stores/ directory entirely. Verify the entire codebase is Zustand-free and remove the zustand package if nothing depends on it.

Purpose: Complete the Zustand-to-Context migration that was partially done. The shim files (auth-store.ts, notification-ui-store.ts, stores/index.ts) are backward-compatibility wrappers that add indirection. After Plan 01-01 removed notes-store.ts and notes references from stores/index.ts, only auth and notification-ui shims remain. This plan eliminates them and ensures the zustand dependency is fully cleaned up.

Output: Zero imports from `@/stores/*` anywhere in the codebase. The `stores/` directory no longer exists. No direct imports from the `zustand` package remain. Zustand removed from package.json if no longer needed.
</objective>
...
<task type="auto">
  ...
  <action>
    ...
    IMPORTANT: The auth-context exports the exact same named exports as the shim. No API changes needed — only import paths change. Read auth-store.ts to see the full export list and match it against auth-context.tsx.
  </action>
  <verify>
...
<task type="auto">
  <name>Task 2: Delete store shim files, remove stores directory, and clean up zustand dependency</name>
  <files>
    ...
    electron-app/package.json (POSSIBLY MODIFY)
  </files>
  <action>
    ...
    7. **Verify entire codebase is Zustand-free:**
       - Run `grep -rn "from 'zustand'\|from \"zustand\"\|require('zustand')\|require(\"zustand\")" electron-app/src/ --include="*.ts" --include="*.tsx" --include="*.js" --include="*.jsx"` to check if ANY file still directly imports from the `zustand` package.
       - If ZERO results: remove `zustand` from `electron-app/package.json` dependencies (and devDependencies if present). Run `cd electron-app && npm install` to update the lockfile.
       - If results found: document which files still use zustand and why in a comment at the top of the commit message. Do NOT remove the package in that case.
  </action>
  <verify>
    ...
    - `grep -rn "from 'zustand'\|from \"zustand\"" electron-app/src/ --include="*.ts" --include="*.tsx"` returns zero results — confirming no direct zustand imports remain
    - If zustand was removed from package.json: `node -e "const p = require('./electron-app/package.json'); console.log(p.dependencies?.zustand || p.devDependencies?.zustand || 'NOT FOUND')"` prints `NOT FOUND`
    - `cd electron-app && npm run typecheck` passes
    - `cd electron-app && npm run lint` passes (no unused imports warnings)
  </verify>
  <done>
    The stores/ directory is completely removed. No Zustand shim files exist. No import anywhere references @/stores. The tsconfig path alias for @/stores is removed if it existed. The entire codebase has been verified to have zero direct imports from the 'zustand' package. If no zustand usage was found, the zustand package has been removed from package.json dependencies. Frontend compiles and lints cleanly.
  </done>
</task>
...
3. Zero zustand imports: `grep -rn "from 'zustand'\|from \"zustand\"" electron-app/src/ --include="*.ts" --include="*.tsx"` returns empty
4. TypeScript: `cd electron-app && npm run typecheck` passes
5. Lint: `cd electron-app && npm run lint` passes
6. Auth still works: imports from `@/contexts/auth-context` resolve correctly
</verification>
...
- No file in electron-app/src/ directly imports from the 'zustand' package
- If no zustand usage found, zustand is removed from package.json dependencies
