...
<task type="auto">
  ...
  <action>
    ...
    **Preload (`index.ts`):**
    ...
    4. Add two new methods to the exposed API object (follow existing patterns like `onMaximizedChange` or `onWebSocketMessage`):
       - `onBeforeQuit: (callback: () => void) => () => void` -- registers callback for `before-quit-save` IPC channel, returns cleanup function
       - `confirmQuitSave: () => void` -- sends `quit-save-complete` IPC to main process
  ...
  <done>
    Electron main process intercepts close, sends IPC to renderer, waits up to 3 seconds. Preload exposes onBeforeQuit and confirmQuitSave. useSaveOnQuit hook wires the IPC to saveNow(). Save-on-unmount pattern documented or provided as utility hook.
  </done>
</task>
...
2. Preload exposes `onBeforeQuit` and `confirmQuitSave` following existing IPC bridge patterns
...
- All wiring uses existing preload IPC patterns (no new patterns invented)
</success_criteria>
