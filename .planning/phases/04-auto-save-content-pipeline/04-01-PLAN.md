...
<task type="auto">
  ...
  <action>
    ...
    2. In `document_service.py`, add a `save_document_content` function:
       ...
       - Do NOT manually set `updated_at` -- the Document model uses SQLAlchemy's `onupdate` parameter which auto-updates on commit
       - Do NOT set `updated_by` -- this column does not exist on the Document model (only `created_by` exists)
  ...
  <done>
    PUT /documents/{id}/content endpoint exists, accepts content_json + row_version, returns 409 on version mismatch, updates document content and increments row_version on success. updated_at auto-updates via SQLAlchemy onupdate.
  </done>
</task>
...
4. Backend: Service function does NOT set `updated_by` (column doesn't exist) and does NOT manually set `updated_at` (auto-updates via SQLAlchemy onupdate)
5. Frontend: `useSaveDocumentContent` mutation exists in use-queries.ts
6. Frontend: `useAutoSave` hook exports `saveNow`, `isDirty`, `saveStatus`
