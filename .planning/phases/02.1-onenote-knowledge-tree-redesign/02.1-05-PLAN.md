...
- **DocumentEditor component exists** from Phase 3 (completed 2026-02-01) at `electron-app/src/renderer/components/knowledge/document-editor.tsx`. It accepts a `documentId` prop and renders the TipTap collaborative editor.
- **Application detail page** (`pages/applications/[id].tsx`) currently uses a `showArchive` boolean state to toggle between Projects and Archive views. It has a tab-like button bar (lines ~862-897). This plan refactors `showArchive` to a three-value `activeView` state.
- **Project detail page** (`pages/projects/[id].tsx`) currently has no sub-tabs — it renders the KanbanBoard directly. This plan adds a tab bar above the board.
</objective>
...
<task type="auto">
  <name>Task 2: Add Knowledge tab to Application detail page</name>
  <files>
    electron-app/src/renderer/pages/applications/[id].tsx
  </files>
  <action>
    The page currently has a tab bar with "Projects" and "Archive" tabs (lines ~862-897) using a `showArchive` boolean state. Refactor to a three-value state and add "Knowledge" tab:

    1. Replace `showArchive` boolean with a string state:
       ```typescript
       const [activeView, setActiveView] = useState<'projects' | 'archive' | 'knowledge'>('projects')
       ```
    2. Replace ALL existing `showArchive` logic with `activeView`:
       - `showArchive` → `activeView === 'archive'`
       - `!showArchive` → `activeView === 'projects'`
       - `setShowArchive(true)` → `setActiveView('archive')`
       - `setShowArchive(false)` → `setActiveView('projects')`
       - Line ~900: `{!showArchive ? (` → `{activeView === 'projects' ? (`
       - Line ~1059: `showArchive` in the ternary → `activeView === 'archive'`
    3. Add a "Knowledge" tab button to the existing tab bar:
       ```tsx
       <button
       ...
       ```
    4. Add the Knowledge tab content:
       ```tsx
       {activeView === 'knowledge' && (
    ...
    IMPORTANT: Search for ALL references to `showArchive` in the file and update them. Missing any will cause TypeScript errors since the variable no longer exists.
  </action>
  <verify>
    1. `cd electron-app && npx tsc --noEmit --pretty 2>&1 | head -30` — zero errors
    2. `grep -n "Knowledge" electron-app/src/renderer/pages/applications/\\[id\\].tsx` — tab and panel references found
    3. `grep -n "showArchive" electron-app/src/renderer/pages/applications/\\[id\\].tsx` — no matches (fully replaced)
  </verify>
  <done>
    Application detail page has Projects / Archive / Knowledge tabs with KnowledgePanel embedded. showArchive boolean fully replaced with activeView string. No TypeScript errors.
  </done>
</task>

<task type="auto">
  <name>Task 3: Add Knowledge tab to Project detail page</name>
  <files>
    electron-app/src/renderer/pages/projects/[id].tsx
  </files>
  <action>
    The project page currently has no sub-tabs — it renders the KanbanBoard directly. Add a tab bar at the top of the board section:
  ...
  <verify>
    1. `cd electron-app && npx tsc --noEmit --pretty 2>&1 | head -30` — zero errors
    2. `grep -n "Knowledge" electron-app/src/renderer/pages/projects/\\[id\\].tsx` — tab and panel references found
  </verify>
  <done>
    Project detail page has Board / Knowledge tabs with KnowledgePanel embedded. No TypeScript errors.
  </done>
</task>
