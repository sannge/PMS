...
must_haves:
  truths:
    ...
    - "Tabs UI component (shadcn/ui pattern) exists and renders accessible Radix Tabs"
  artifacts:
...
<task type="auto">
  ...
  <action>
    **KnowledgeBaseContext refactor:**
    ...
    4. Update all localStorage key constants to use the prefix:
       - In the provider init function, use `storagePrefix ?? 'kb-'` as a prefix for all keys
       - Pass prefix through the reducer init and persistence effects
