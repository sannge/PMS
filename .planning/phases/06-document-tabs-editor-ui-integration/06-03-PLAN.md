...
**Editor instance wiring (REQUIRED):** The DocumentStatusBar needs an editor instance for word count. The DocumentEditor currently creates its editor internally via `useEditor` and does not expose it. Apply these changes:

1. **Add `onEditorReady` prop to DocumentEditorProps** in `editor-types.ts`:
   ```typescript
   onEditorReady?: (editor: Editor) => void
   ```

2. **In `document-editor.tsx`:**
   - Accept `onEditorReady` from props
   - Add a useEffect after the `useEditor` call: `useEffect(() => { if (editor) onEditorReady?.(editor) }, [editor, onEditorReady])`
   - **Remove the internal `EditorStatusBar` component and its rendering** (lines ~22-37 and the `<EditorStatusBar editor={editor} />` in the JSX). Phase 6's external DocumentStatusBar supersedes it with "last saved" timestamp addition.
   - Also remove the border/rounded-lg from the outer `<div>` since the editor will be embedded in DocumentPanel's layout (change to `className={cn('overflow-hidden bg-background', className)}`)

3. **In DocumentPanel:** Store editor in local state via `useState<Editor | null>(null)`. Pass `onEditorReady={setEditor}` to DocumentEditor. Pass the stored editor to DocumentStatusBar.
...
<div key={activeTabId} className="flex-1 flex flex-col h-full min-w-0">
  ...
  <DocumentStatusBar editor={editorInstance} lastSavedAt={document.updated_at} />
</div>
```
