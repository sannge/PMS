...
3. **Auto-save setup:**
   ...
   ```
   Use `useAutoSave({ documentId, editor content tracking... })` -- this needs careful wiring:
   - The `useAutoSave` hook needs to track content changes from the editor
   - Pass `document?.row_version` as the initial row version
   - The `onChange` callback from DocumentEditor updates the auto-save content tracking

4. **Tab dirty sync:** Sync auto-save dirty state to tab bar:
   ```typescript
   const { setTabDirty } = useKnowledgeBase()
   // On editor update, check isDirty and sync to tab
   useEffect(() => {
   ...
   ```
   Also sync dirty=true on editor content changes.
...
- Skeleton from `@/components/ui/skeleton`
  ...
  <verify>
Run `npx tsc --noEmit` from electron-app/ to verify no type errors. Grep for `DocumentPanel` in the new file to confirm export. Grep for `key=.*activeTabId` or `key={activeTabId}` in the file to confirm key-based remount pattern. Grep for `useAutoSave` to confirm auto-save integration.
  </verify>
  <done>
...
**editor-types.ts -- Add onEditorReady prop:**

Add to `DocumentEditorProps` interface:
```typescript
/** Callback when TipTap editor instance is ready (for external status bar) */
onEditorReady?: (editor: Editor) => void
```

The import of `Editor` from `@tiptap/react` already exists at the top of the file. Also import `Editor` from `@tiptap/core` if needed (check existing import -- the file imports from `@tiptap/react` which re-exports `Editor`).

**document-editor.tsx -- Call onEditorReady:**

Add a useEffect after the `useEditor` call (after line 92):
```typescript
useEffect(() => {
  if (editor && onEditorReady) {
    onEditorReady(editor)
...
Add `onEditorReady` to the destructured props in the function signature (it's already in DocumentEditorProps from the type change above).
  </action>
  <verify>
Run `npx tsc --noEmit` from electron-app/ to verify no type errors. Grep for `DocumentTabBar` in notes/index.tsx to confirm tab bar is rendered. Grep for `openTab` in folder-tree.tsx to confirm sidebar uses tab opening. Grep for `onEditorReady` in editor-types.ts and document-editor.tsx to confirm the prop is added and called.
  </verify>
  <done>
Notes page renders DocumentTabBar + DocumentPanel. Sidebar document clicks open tabs. New document creation opens tabs. Editor exposes onEditorReady callback for external status bar. The complete Phase 6 feature is functional end-to-end.
  </done>
</task>
