...
```
- Uses `window.electronAPI.post` to POST `/api/documents/{documentId}/tags` with JSON body `{ tag_id: tagId }`
- Auth headers from `useAuthStore` (reuse existing `getAuthHeaders`)
...
- Backend returns 201 with TagAssignmentResponse body, but mutation return can be void (ignore response body)
...
- Same auth and invalidation pattern as assign
...
Note: The `parseApiError` and `getAuthHeaders` helper functions already exist in use-document-tags.ts. Reuse them. Also add `useMutation`, `useQueryClient` imports from `@tanstack/react-query`.
  </action>
