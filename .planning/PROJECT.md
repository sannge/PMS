...
**Scale target:** 5,000 concurrent users, tens of thousands of documents. Database queries need pagination and indexing. Auto-save writes ~50-100KB TipTap JSON per save (images stored separately in MinIO).
...
- Plain text → generated server-side on save (for search indexing)
- Auto-save only writes document JSON (~50-100KB), not image binaries
