...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "(CASE WHEN application_id IS NOT NULL THEN 1 ELSE 0 END + CASE WHEN user_id IS NOT NULL THEN 1 ELSE 0 END) = 1",
    ...
    op.create_index(
        op.f("ix_DocumentTagAssignments_document_id"), "DocumentTagAssignments", ["document_id"], unique=False
    )
