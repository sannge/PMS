...
def upgrade() -> None:
    ...
    op.create_index("ix_Tasks_project_status", "Tasks", ["project_id", "status"], postgresql_using="btree")

    # Used by: Task ordering within project/status columns
    op.create_index("ix_Tasks_project_rank", "Tasks", ["project_id", "task_rank"], postgresql_using="btree")

    # Used by: "My tasks" queries filtering by assignee and status
    op.create_index("ix_Tasks_assignee_status", "Tasks", ["assignee_id", "status"], postgresql_using="btree")

    # Used by: Task status ID lookups (new unified status system)
    op.create_index("ix_Tasks_project_status_id", "Tasks", ["project_id", "task_status_id"], postgresql_using="btree")

    # Comments - Frequently filtered by task with time ordering
    op.create_index("ix_Comments_task_created", "Comments", ["task_id", "created_at"], postgresql_using="btree")
    ...
    op.create_index("ix_Checklists_task_rank", "Checklists", ["task_id", "rank"], postgresql_using="btree")

    # ChecklistItems - Checklist lookup with rank ordering
    op.create_index(
        "ix_ChecklistItems_checklist_rank", "ChecklistItems", ["checklist_id", "rank"], postgresql_using="btree"
    )

    # ApplicationMembers - Application + role for permission checks
    op.create_index(
        "ix_ApplicationMembers_app_role", "ApplicationMembers", ["application_id", "role"], postgresql_using="btree"
    )

    # ApplicationMembers - User + application for membership lookups
    op.create_index(
        "ix_ApplicationMembers_user_app", "ApplicationMembers", ["user_id", "application_id"], postgresql_using="btree"
    )

    # ProjectMembers - User + project for permission checks
    op.create_index(
        "ix_ProjectMembers_user_project", "ProjectMembers", ["user_id", "project_id"], postgresql_using="btree"
    )
    ...
    op.create_index(
        "ix_Notifications_unread", "Notifications", ["user_id", "created_at"], postgresql_where=text("is_read = false")
    )
    ...
    op.create_index(
        "ix_Comments_active", "Comments", ["task_id", "created_at"], postgresql_where=text("is_deleted = false")
    )
