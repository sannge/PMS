...
def upgrade() -> None:
    ...
    op.execute(
        "CREATE UNIQUE INDEX uq_ai_providers_global_type "
        "ON \"AiProviders\" (provider_type) "
        "WHERE scope = 'global'"
    )
    ...
    op.execute(
        "CREATE UNIQUE INDEX uq_ai_providers_user_type "
        "ON \"AiProviders\" (provider_type, user_id) "
        "WHERE scope = 'user'"
    )
