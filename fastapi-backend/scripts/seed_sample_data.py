...
COMMENTS = [
    ...
    (
        ...
        _uuid(103),
        0,
        "Started reviewing the PR. Endpoint design looks solid — just need to verify cursor pagination handles empty results and last-page edge cases.",
        5,
    ),
    ...
    (
        ...
        _uuid(103),
        0,
        "LGTM on pagination tests. One last ask: add 422 validation error examples to the OpenAPI docs before merging.",
        3,
    ),
    ...
    (
        ...
        _uuid(104),
        3,
        "This is causing significant slowdown in staging load tests — 300ms on task list vs expected 50ms. Needs urgent attention.",
        7,
    ),
    (
        _uuid(1005),
        _uuid(104),
        0,
        "Confirmed. Fix is to add selectinload(Task.assignee) and selectinload(Task.task_status) in the list query. Working on it now.",
        6,
    ),
    ...
    (
        ...
        _uuid(106),
        0,
        "Initial impl is in feature/ws-manager. Uses Redis pub/sub channels per project. Handles reconnect with exponential backoff.",
        4,
    ),
    (
        _uuid(1007),
        _uuid(106),
        2,
        "Looks good overall. One concern: what happens when Redis goes down mid-session? Do clients get a notification?",
        3,
    ),
    (
        _uuid(1008),
        _uuid(106),
        0,
        "Good catch! Added graceful degradation — clients receive a connection_lost event and can fall back to polling.",
        2,
    ),
    ...
    (
        ...
        _uuid(108),
        0,
        "Tests pass locally but fail on CI. Suspect missing env var for the test database URL.",
        8,
    ),
    (
        _uuid(1010),
        _uuid(108),
        0,
        "Found it — TEST_DATABASE_URL wasn't exported in the CI env template. Tests now pass, but teardown still leaves orphaned rows.",
        7,
    ),
    ...
    (
        ...
        _uuid(113),
        3,
        "Reproducible 100% on Pixel 7 running Android 14 QPR2. Stack trace points to BiometricPrompt callback null dereference.",
        10,
    ),
    (
        _uuid(1012),
        _uuid(113),
        1,
        "Taking ownership. Android 14 changed the BiometricPrompt API — the old callback signature is now deprecated and throws on QPR2.",
        9,
    ),
    ...
    (
        ...
        _uuid(113),
        1,
        "Fix is ready and tested on Pixel 7 emulator. PR up for review. Will need QA sign-off on a physical device before merge.",
        6,
    ),
    ...
    (
        ...
        _uuid(116),
        3,
        "Avatar background stays white in dark mode. CSS variable --avatar-bg is not connected to the theme token.",
        5,
    ),
    (
        _uuid(1016),
        _uuid(116),
        1,
        "On it. Missing data-theme attribute propagation in the Profile component root. Simple one-liner fix.",
        4,
    ),
    ...
    (
        ...
        _uuid(117),
        1,
        "Initial audit: FlatList re-renders on every store update because keyExtractor returns index not id. Easy fix, big impact.",
        6,
    ),
    (
        _uuid(1018),
        _uuid(117),
        6,
        "Also spotted a memory leak — useEffect subscription to analytics events is not cleaned up on unmount.",
        5,
    ),
    (
        _uuid(1019),
        _uuid(117),
        1,
        "Both issues fixed in my branch. PR is up. Also added React.memo to the task card component to halve re-render count.",
        4,
    ),
    ...
    (
        ...
        _uuid(123),
        8,
        "Using Kafka consumers with manual offset commit. Failures mid-batch can replay from last committed offset without data loss.",
        5,
    ),
    (
        _uuid(1021),
        _uuid(123),
        5,
        "Good pattern. Cap dead-letter queue retries at 3 to avoid infinite loops. Also consider alerting on DLQ depth.",
        4,
    ),
    ...
    (
        ...
        _uuid(125),
        8,
        "Aggregation query runs in ~450ms on 10M rows. We need it under 200ms to hit the dashboard SLO.",
        6,
    ),
    (
        _uuid(1023),
        _uuid(125),
        5,
        "Added a partial index on created_at for the last 30 days. Down to 180ms on the same dataset. PR ready.",
        5,
    ),
    (
        _uuid(1024),
        _uuid(125),
        8,
        "Excellent. Approving the PR. Let's monitor query time in production after deploy.",
        4,
    ),
    ...
    (
        ...
        _uuid(126),
        8,
        "Memory grows ~2MB/min under load. Stream processor accumulates state in a HashMap for deduplication but never evicts.",
        6,
    ),
    (
        _uuid(1026),
        _uuid(126),
        5,
        "Confirmed root cause. Implementing LRU eviction policy with 10k entry limit. Will deploy to staging tomorrow for validation.",
        5,
    ),
    ...
    (
        ...
        _uuid(132),
        4,
        "Using @dnd-kit SortableContext. Main challenge is persisting widget order to backend efficiently on drop end.",
        5,
    ),
    (
        _uuid(1028),
        _uuid(132),
        2,
        "Debounce the persist call — we don't want an API request on every intermediate drag position, only on drop.",
        4,
    ),
    ...
    (
        ...
        _uuid(133),
        2,
        "Almost done. Chart colours are the last blocker — Recharts doesn't auto-inherit CSS vars, need explicit prop passing.",
        4,
    ),
    (
        _uuid(1030),
        _uuid(133),
        4,
        "Pass colours as props derived from a useTheme() hook. That keeps the chart decoupled from CSS specifics.",
        3,
    ),
    ...
    (
        ...
        _uuid(138),
        6,
        "Main bundle includes all Recharts components even for screens without charts. Lazy loading will cut initial load ~40%.",
        6,
    ),
    (
        _uuid(1032),
        _uuid(138),
        2,
        "Use React.lazy + Suspense with a skeleton fallback. Make sure to test the Suspense boundary on throttled 3G.",
        5,
    ),
    ...
    (
        ...
        _uuid(144),
        4,
        "Profiling shows aggregation service holds event references in a HashMap for dedup — they accumulate indefinitely in long-running workers.",
        5,
    ),
    (
        _uuid(1034),
        _uuid(144),
        2,
        "Suggest a TTL-based Bloom filter instead of HashMap. Same false-positive rate for dedup, fraction of the memory footprint.",
        4,
    ),
    (
        _uuid(1035),
        _uuid(144),
        4,
        "Implemented TTL Bloom filter (1h TTL, 0.1% FPR). Memory growth dropped to near-zero in 4-hour load test. PR up.",
        3,
    ),
    ...
    (
        ...
        _uuid(148),
        2,
        "Cohort query schema is designed. Building the SQL generation layer — supporting up to 5 sequential steps per cohort.",
        6,
    ),
    (
        _uuid(1037),
        _uuid(148),
        6,
        "What is the expected response time for a cohort with 500k users and a 6-month window?",
        5,
    ),
    (
        _uuid(1038),
        _uuid(148),
        2,
        "~2-3s cold, <500ms with incremental caching (computing daily deltas and storing them). Implementing that now.",
        4,
    ),
    ...
    (
        ...
        _uuid(154),
        8,
        "Tests that hit the DB are race-conditioning when running in parallel — multiple workers share the same DB schema.",
        7,
    ),
    (
        _uuid(1040),
        _uuid(154),
        0,
        "Using pytest-xdist with dist=each and a worker-scoped DB fixture. Each worker gets its own schema prefix. Should eliminate races.",
        6,
    ),
    ...
    (
        ...
        _uuid(157),
        8,
        "Split suite into 4 shards by file size. CI time dropped from 18 min to under 5 min. Shard config in .github/workflows/ci.yml.",
        5,
    ),
    (
        _uuid(1042),
        _uuid(157),
        0,
        "Nice work! Confirm coverage reports from each shard are being merged — otherwise we'll report inflated coverage gaps.",
        4,
    ),
    ...
    (
        ...
        _uuid(167),
        9,
        "Alertmanager config on staging drifted from prod — routing trees differ. Someone edited the config file manually without committing.",
        8,
    ),
    (
        _uuid(1044),
        _uuid(167),
        8,
        "Identified the change: an inhibition rule was added for the Redis alerts but never pushed to git. Adding alertmanager config to GitOps repo and making the file read-only on all envs.",
        7,
    ),
    ...
    (
        ...
        _uuid(174),
        9,
        "Redis Sentinel failover is taking 45 seconds, causing cascading connection timeouts across all services. Acceptable is under 10 seconds.",
        5,
    ),
    (
        _uuid(1046),
        _uuid(174),
        4,
        "Increased sentinel heartbeat frequency (down-after-milliseconds from 30000 to 5000) and reduced failover-timeout. Deploying to staging.",
        4,
    ),
    (
        _uuid(1047),
        _uuid(174),
        9,
        "Staging still showing 25s failover. Sentinel quorum requires 2/3 but one sentinel is on a high-latency node. Investigating topology.",
        3,
    ),
    ...
    (
        ...
        _uuid(177),
        9,
        "Using cert-manager for automatic certificate rotation (24h TTL). All service identities provisioned via SPIFFE/SPIRE.",
        6,
    ),
    (
        _uuid(1049),
        _uuid(177),
        4,
        "Solid setup. Make sure certificate rotation doesn't cause connection drops — test with a forced rotation in staging first.",
        5,
    ),
]
...
async def seed() -> None:
    ...
    async with async_session_maker() as session:
        ...
        # Flush projects first so FK references from TaskStatuses resolve
        await session.flush()
        ...
        status_lookup: dict[uuid.UUID, dict[str, uuid.UUID]] = {
            pid: {s.name: s.id for s in sts} for pid, sts in all_statuses.items()
        }

        task_numbers: dict[int, int] = {i: 1 for i in range(len(PROJECTS))}
        agg_counts: dict[uuid.UUID, dict[str, int]] = {
            p["id"]: {"total": 0, "todo": 0, "active": 0, "review": 0, "issue": 0, "done": 0} for p in PROJECTS
        }
        STATUS_AGG = {
            "Todo": "todo",
            "In Progress": "active",
        ...
        for i, p in enumerate(PROJECTS):
            ...
            await session.execute(
                ...
                .values(
                    ...
                    review_tasks=c["review"],
                    issue_tasks=c["issue"],
                    done_tasks=c["done"],
                    updated_at=_ts(14),
                )
            )
            await session.execute(update(Project).where(Project.id == pid).values(next_task_number=task_numbers[i]))
        await session.flush()
        ...
        for task_id, (total, done) in task_cl_totals.items():
            await session.execute(
                update(Task).where(Task.id == task_id).values(checklist_total=total, checklist_done=done)
            )
        await session.flush()
        ...
        for f_id, name, scope_type, scope_idx, parent_id, creator_idx, day in FOLDERS:
            ...
            mat_path = f"/{f_id}/" if parent_id is None else f"/{parent_id}/{f_id}/"
...
async def main() -> None:
    ...
    overdue = sum(1 for (_, _, _, _, _, status, _, _, _, due_off, _) in TASKS if status != "Done" and due_off < 0)
